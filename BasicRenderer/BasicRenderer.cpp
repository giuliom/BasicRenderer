...
namespace BasicRenderer
{
	...
	const std::shared_ptr<const FrameBuffer> Renderer::Render(int width, int height, World& scene, RenderingMode mode, ShadingMode shading, int samplesPerPixel, int maxBounces)
	{
	...
	const std::shared_ptr<const FrameBuffer> Renderer::RayTracing(int width, int height, World& scene, int pixelSamples, int bounces, Color(Material::* shading)(const World& w, const Vector3& pos, const Vector3& nrml))
	{
	...
	Color Renderer::RayTrace(const Ray& ray, World& scene, int bounces, Color(Material::* shading)(const World& w, const Vector3& pos, const Vector3& nrml))
	{
	...
	void Renderer::DrawObject(const Primitive* primitive, const World& scene, Color(Material::* shading)(const World& w, const Vector3& pos, const Vector3& nrml))
	{
	...
	inline void Renderer::PerspectiveDivide(Face& f) const
	{
	...
	inline void Renderer::NormalizedToScreenSpace(Face& f) const
	{
	...
	inline int Renderer::Clip(const Face& f, Face(&clippedFaces)[4]) const
	{
		int nfaces = 0;
	...
	inline int Renderer::ClipEdge(const Vertex& v0, const Vertex& v1, Vertex(&vertices)[6], int index) const
	{
		assert(index < 5);
	...
	inline bool Renderer::CullFace(const Face& f) const
	{
	...
	inline Vector4 Renderer::BoundingBox(const Face& f) const
	{
